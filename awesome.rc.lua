...
tagnames = { "comms", "web", "vms", "terms", "notes", "misc" }
taglayouts = {
    awful.layout.suit.tile.left,
    awful.layout.suit.tile,
    awful.layout.suit.tile,
...
for s = 1, screen.count() do
	...
	currenttag = tag(tagnames[tagnumber])
	if tagnumber == 1 then
		currenttag.selected = true
	end
	tags[s][tagnumber] = currenttag
	-- Add tags to screen one by one, giving layouts at the same time
...
memwidget:set_gradient_colors({ "#5656A0", "#8875B1", "#A0A0F0" })
...
cpuwidget:set_gradient_colors({ "#56A056", "#88B175", "#A0F0A0" })
